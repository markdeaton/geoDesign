...
/*        string token = getTokenFromConfigFile(uri);
        ...
        if (serverResponse != null) {
            ...
            using (Stream byteStream = serverResponse.GetResponseStream())
            {
                ...
                {
                    // Legacy code depends on serverResponse.ContentLength, which is unreliable.
                    // Replaced this with a loop that reads data until no more is available.
                    ...
                    // Tell client not to cache the image since it's dynamic
                    response.CacheControl = "no-cache"; 
                    response.OutputStream.Write(arrBuffer, 0, arrBuffer.Length);
                }
            }
        }
